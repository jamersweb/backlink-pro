<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Fix table name: ensure domain_onboardings exists (rename from domain_onboarding or create).
     */
    public function up(): void
    {
        if (Schema::hasTable('domain_onboarding') && !Schema::hasTable('domain_onboardings')) {
            Schema::rename('domain_onboarding', 'domain_onboardings');
            return;
        }

        if (Schema::hasTable('domain_onboardings')) {
            return;
        }

        Schema::create('domain_onboardings', function (Blueprint $table) {
            $table->id();
            $table->foreignId('domain_id')->constrained('domains')->onDelete('cascade')->unique();
            $table->foreignId('user_id')->constrained('users')->onDelete('cascade');
            $table->enum('status', ['in_progress', 'completed'])->default('in_progress');
            $table->string('current_step')->nullable();
            $table->json('steps_json');
            $table->timestamps();

            $table->index(['user_id', 'status']);
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        if (Schema::hasTable('domain_onboardings')) {
            Schema::dropIfExists('domain_onboardings');
        }
    }
};
