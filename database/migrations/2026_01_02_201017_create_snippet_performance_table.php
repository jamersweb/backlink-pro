...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('snippet_performance')) {
            // Check if table has all required columns - if so, skip migration
            $requiredColumns = ['id', 'domain_id', 'date', 'path', 'avg_load_ms', 'avg_ttfb_ms', 'samples', 'created_at', 'updated_at'];
            $hasAllColumns = true;
            foreach ($requiredColumns as $column) {
                if (!Schema::hasColumn('snippet_performance', $column)) {
                    $hasAllColumns = false;
                    break;
                }
            }
            ...
            try {
                $constraints = DB::select("
                    ...
                    AND TABLE_NAME = 'snippet_performance' 
                    AND REFERENCED_TABLE_NAME IS NOT NULL
                ");
                
                foreach ($constraints as $constraint) {
                    // Drop existing foreign key constraint if it exists
                    DB::statement("ALTER TABLE snippet_performance DROP FOREIGN KEY {$constraint->CONSTRAINT_NAME}");
                }
            } catch (\Exception $e) {
            ...
            Schema::dropIfExists('snippet_performance');
        }
