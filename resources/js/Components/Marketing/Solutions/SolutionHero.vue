<template>
    <section class="solution-hero py-20 md:py-32 relative overflow-hidden">
        <div class="bg-grid-pattern absolute inset-0 opacity-30"></div>
        <div class="marketing-container relative z-10">
            <!-- Breadcrumb -->
            <nav class="mb-8" aria-label="Breadcrumb">
                <ol class="flex items-center gap-2 text-sm text-muted">
                    <li><Link href="/solutions" class="hover:text-text">Solutions</Link></li>
                    <li>/</li>
                    <li class="text-text">{{ solution.name }}</li>
                </ol>
            </nav>
            ...
            <div class="grid md:grid-cols-2 gap-12 items-center">
                ...
                <div data-reveal>
                    ...
                    <div class="flex flex-wrap gap-4 mb-6">
                        ...
                        <button @click="$emit('demo-open')" class="btn-ghost text-lg px-8 py-4">
                            {{ solution.cta.tertiary }}
                        </button>
                    </div>
                </div>
                    ...
                    <div class="marketing-card p-6">
                        <div class="aspect-video bg-surface2 rounded-lg overflow-hidden border border-border relative">
                            <video
                                ...
                                ref="videoRef"
                                :poster="solution.heroMedia.poster"
                                loop
                                muted
                                playsinline
                                class="w-full h-full object-cover"
                            >
                                <source :src="solution.heroMedia.video" type="video/mp4" />
                            </video>
                            <div
                                v-else
...
onMounted(() => {
    ...
    if (!isMobile.value) {
        ...
        const card = document.querySelector('.solution-hero .marketing-card');
        if (card) {
