...
const providers = [
    {
        key: 'google',
        label: 'Google',
        accentClass: 'group-hover:border-[#4285F4]/60 group-hover:shadow-[#4285F4]/20',
        icon: (
    ...
    },
    {
        key: 'microsoft',
        label: 'Microsoft',
        accentClass: 'group-hover:border-[#00A4EF]/60 group-hover:shadow-[#00A4EF]/20',
        icon: (
            <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
    ...
    },
    {
        key: 'github',
        label: 'GitHub',
        accentClass: 'group-hover:border-white/30 group-hover:shadow-white/10',
        icon: (
    ...
    },
    {
        key: 'facebook',
        label: 'Facebook',
        accentClass: 'group-hover:border-[#1877F2]/60 group-hover:shadow-[#1877F2]/20',
        icon: (
        ...
        <div className={className}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {providers.map((provider) => {
                    ...
                    const isFacebook = provider.key === 'facebook';

                    return (
                        <a
                            key={provider.key}
                            href={buildAuthRedirectUrl(provider.key, currentUrl)}
                            className={`group relative inline-flex items-center justify-between rounded-xl border border-white/15 bg-gradient-to-r from-[#0E1525] to-[#0B0F14] px-4 py-3.5 text-sm font-semibold text-[#E5E7EB] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg ${provider.accentClass} ${isFacebook ? 'sm:col-span-2' : ''}`}
                        >
                            <span className="inline-flex items-center gap-3">
                                <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-white/15 bg-[#0B0F14] text-white">
                                    {provider.icon}
                                </span>
                                <span>{provider.label}</span>
                            </span>
                            <span className={`text-[10px] uppercase tracking-[0.18em] ${isConfigured ? 'text-[#12B76A]' : 'text-[#F79009]'}`}>
