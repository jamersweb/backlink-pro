        ...
        <AdminLayout header="Pricing Plans">
            <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <p className="text-[var(--admin-text-muted)] mt-1">Manage your subscription plans and pricing</p>
                    </div>
                    <Link
                    ...
                    <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-5 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-[var(--admin-text-muted)]">Total Plans</p>
                            </div>
                        </div>
                    </div>
                    <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-5 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold text-[var(--admin-text)]">{plans.filter(p => p.is_active).length}</p>
                                <p className="text-sm text-[var(--admin-text-muted)]">Active Plans</p>
                            </div>
                        </div>
                    </div>
                    <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-xl p-5 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold text-[var(--admin-text)]">{plans.filter(p => p.is_public).length}</p>
                                <p className="text-sm text-[var(--admin-text-muted)]">Public Plans</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    {plans.map((plan) => (
                        <div
                            ...
                            className={`relative overflow-hidden rounded-2xl border transition-all duration-300 ${
                                plan.is_highlighted 
                                    ? 'bg-gradient-to-br from-[var(--admin-surface)] via-[var(--admin-hover-bg)] to-[var(--admin-surface)] border-[#2F6BFF]/50 shadow-xl shadow-[#2F6BFF]/10' 
                                    : 'bg-[var(--admin-surface)] border-[var(--admin-border)] hover:border-[var(--admin-hover-border)]'
                            }`}
                        >
                            {/* Highlight Badge */}
                            {plan.is_highlighted && (
                                <div className="absolute top-0 right-0">
                                    <div className="bg-gradient-to-r from-[#2F6BFF] to-[#B6F400] text-white dark:text-[#0B0F14] text-xs font-bold px-3 py-1 rounded-bl-lg">
                                        {plan.badge || 'Popular'}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <h3 className="text-xl font-bold text-[var(--admin-text)]">{plan.name}</h3>
                                        {plan.tagline && (
                                            <p className="text-sm text-[var(--admin-text-muted)] mt-1">{plan.tagline}</p>
                                        )}
                                    </div>
                                ...
                                <div className="mb-6">
                                    {plan.price_monthly !== null ? (
                                        <div className="flex items-baseline gap-1">
                                            <span className="text-4xl font-bold text-[var(--admin-text)]">${plan.price_monthly}</span>
                                            <span className="text-[var(--admin-text-muted)]">/month</span>
                                        </div>
                                    ) : (
                                    ...
                                    <div className="space-y-2 mb-6">
                                        {plan.display_limits.map((limit, idx) => (
                                            <div key={idx} className="flex items-center justify-between text-sm">
                                                <span className="text-[var(--admin-text-muted)]">{limit.label}</span>
                                                <span className="text-[var(--admin-text)] font-medium">{limit.value}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                {plan.includes && plan.includes.length > 0 && (
                                    <div className="space-y-2 mb-6 pt-4 border-t border-[var(--admin-border)]">
                                        {plan.includes.slice(0, 4).map((feature, idx) => (
                                            <div key={idx} className="flex items-center gap-2 text-sm">
                                                <i className="bi bi-check2 text-[#12B76A]"></i>
                                                <span className="text-[var(--admin-text-muted)]">{feature}</span>
                                            </div>
                                        ))}
                                        {plan.includes.length > 4 && (
                                            <p className="text-xs text-[var(--admin-text-muted)]">+{plan.includes.length - 4} more features</p>
                                        )}
                                    </div>
                                )}
                                ...
                                <div className="flex items-center gap-3 pt-4 border-t border-[var(--admin-border)]">
                                    <Link
                                        ...
                                        className="flex-1 px-4 py-2 bg-[var(--admin-hover-bg)] hover:bg-[var(--admin-surface-2)] text-[var(--admin-text)] rounded-lg text-sm font-medium text-center transition-colors border border-[var(--admin-border)]"
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                                            plan.is_public
                                                ...
                                                : 'bg-[var(--admin-hover-bg)] hover:bg-[var(--admin-surface-2)] text-[var(--admin-text-muted)] border border-[var(--admin-border)]'
                                        }`}
                    ...
                    <Card variant="elevated">
                        <div className="text-center py-12">
                            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[var(--admin-hover-bg)] border border-[var(--admin-border)] mb-4">
                                <i className="bi bi-box-seam text-3xl text-[var(--admin-text-muted)]"></i>
                            </div>
                            <h3 className="text-lg font-medium text-[var(--admin-text)] mb-2">No plans yet</h3>
                            <p className="text-[var(--admin-text-muted)] mb-6">Create your first pricing plan to get started</p>
                            <Link
                                href="/admin/plans/create"
